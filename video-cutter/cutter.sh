...
for element in "${arrays[@]}"; do
    ...
    if [[ -n "${array[0]}" && ${#array[@]} -eq 2 ]]; then
        ...
        echo "$START:$END:$idx"
        echo "ffmpeg -ss $START -to $END -i $ip_video -c copy $DT/$idx$OUTPUT"
        ffmpeg -ss $START -to $END -i "$ip_video" -c copy "$DT/$idx$OUTPUT"
        ((idx++))
    fi
done

echo "<===Total $idx clips are saved in $DT folder===>"
